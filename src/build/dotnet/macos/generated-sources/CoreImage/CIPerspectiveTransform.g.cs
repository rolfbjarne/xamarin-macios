...
#nullable enable
namespace CoreImage {
	/// <summary>A <see cref="T:CoreImage.CIFilter" /> that applies a transform the simulates perspective.</summary><remarks><para>The following example shows this filter in use</para><example><code lang="csharp lang-csharp"><![CDATA[
	/// // Create the CIImage from a file
	/// CIImage heron = CIImage.FromCGImage (UIImage.FromFile ("heron.jpg").CGImage);
	/// 
	/// // Create a CIPerspectiveTransform filter with the input image
	/// var perspective_transform = new CIPerspectiveTransform () {
	/// 	Image = heron
	/// };
	/// 
	/// // Get the altered image from the filter
	/// var output = perspective_transform.OutputImage;
	/// 
	...
	/// </para><para><img href="~/xml/CoreImage/_images/perspective_transform.png" alt="Result of applying the filter." /></para><para>
	...
	/// </para></remarks>
	public partial class CIPerspectiveTransform : CIFilter, ICIPerspectiveTransformProtocol, ICIFourCoordinateGeometryFilterProtocol, ICIFilterProtocol {
		/// <summary>Creates a new <see cref="CIPerspectiveTransform" /> with default values.</summary>
		...
		/// </remarks>
		[BindingImpl (BindingImplOptions.GeneratedCode | BindingImplOptions.Optimizable)]
		protected internal CIPerspectiveTransform (NativeHandle handle) : base (handle)
		{
		}
		...
		protected CIPerspectiveTransform (NSObjectFlag t) : base (t)
		{
		}
		...
		public CIPerspectiveTransform (NSCoder coder) : base (NSObjectFlag.Empty)
		{
			if (coder is null)
		...
		/// <summary>Creates a new <see cref="CIPerspectiveTransform" /> with the specified name.</summary>
		/// <param name="name">The name of the filter to create.</param>
		[BindingImpl (BindingImplOptions.GeneratedCode | BindingImplOptions.Optimizable)]
		public  CIPerspectiveTransform (string name) : base (CreateFilter (name))
		{
		}
		[BindingImpl (BindingImplOptions.GeneratedCode | BindingImplOptions.Optimizable)]
		public CGAffineTransform OutputTransform {
			get {
				var val = ValueForKey ("outputTransform");
				var nsv = (val as NSValue);
				if (nsv is not null)
					return nsv.CGAffineTransformValue;
				return CGAffineTransform.MakeIdentity ();
			}
		}
		// CIPerspectiveTransformProtocol protocol members 
