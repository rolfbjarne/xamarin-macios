	...
	public class UrlProtocolTest {
		...
		public void CanInitWithTask ()
		{
			...
			using (var t = new NSUrlSessionTask ()) {
				ClassicAssert.False (NSUrlProtocol.CanInitWithTask (t), "CanInitWithTask");
			}
		}
		...
		public void Task ()
		{
			...
			using (var p = new CustomProtocol ()) {
				ClassicAssert.Null (p.Task, "Task");
			}
		}
#endif
		...
		public void RegistrarTest ()
		{
			...
			ClassicAssert.IsTrue (TestRuntime.RunAsync (TimeSpan.FromSeconds (10), task, () => CustomUrlProtocol.State >= 5), "Timed out");
			Assert.That (CustomUrlProtocol.State, Is.EqualTo (5), "State");
			ClassicAssert.IsTrue (success, "Success");
		}
