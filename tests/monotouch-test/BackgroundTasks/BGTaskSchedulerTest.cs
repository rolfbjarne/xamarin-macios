...
namespace MonoTouchFixtures.BackgroundTasks {
	...
	public class BGTaskSchedulerTest {
		...
		public void SubmitTaskRequestTest ()
		{
			...
			Assert.That (registered, Is.True, "Task was not registered.");
			...
			Assert.That (error, Is.Null, $"Error submiting request {error}");
			LaunchBGTask ();
			autoResetEvent.WaitOne (300);
			Assert.That (taskWasCalled, Is.True, "Called task.");
		}
	}
}
#endif
