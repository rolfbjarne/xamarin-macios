...
xamarin_generate_conversion_to_managed (id value, MonoType *inputType, MonoType *outputType, MonoMethod *method, guint32 *exception_gchandle, /*SList*/ void **free_list)
{
	...
	if (value) {
		...
		if (isManagedArray) {
			...
			convertedValue = func (value, NULL, managedType, exception_gchandle);
			if (*exception_gchandle != 0)
				goto exception_handling;
...
void *xamarin_nsvalue_to_scnvector3             (NSValue *value, void *ptr, MonoClass *managedType, guint32 *exception_gchandle) {             SCNVector3 *valueptr =             (SCNVector3 *) (ptr ? ptr : xamarin_calloc (sizeof (SCNVector3)));             *valueptr = [value SCNVector3Value];        return valueptr; }
...
xamarin_convert_nsarray_to_managed_with_func (NSArray *array, MonoClass *managedType, xamarin_id_to_managed_func convert, guint32 *exception_gchandle)
{
	...
	for (int i = 0; i < length; i++) {
		valueptr = convert ([array objectAtIndex: i], valueptr, managedType, exception_gchandle);
		if (*exception_gchandle != 0)
			goto exception_handling;
