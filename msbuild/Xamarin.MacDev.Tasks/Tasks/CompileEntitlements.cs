		...
		void ValidateAppEntitlements (MobileProvision? profile, PDictionary requestedEntitlements)
		{
			...
			foreach (var kvp in requestedEntitlements) {
				...
				case "aps-environment":
					...
					if (profile is null) {
						LogEntitlementValidationFailure (onlyWarn, 7135, MSBStrings.E7135, key); // "The app requests the entitlement '{0}', but no provisioning profile has been specified. Please specify the name of the provisioning profile to use with the 'CodesignProvision' property in the project file.
						...
						LogEntitlementValidationFailure (onlyWarn, 7136, MSBStrings.E7136, key, provisioningProfileName); // The app requests the entitlement '{0}', but the provisioning profile '{1}' does not contain this entitlement.
