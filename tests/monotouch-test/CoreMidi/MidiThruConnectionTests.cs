	...
	public class MidiThruConnectionTests {
		...
		public void ConnectionCreateTest ()
		{
			...
			using (var connection = MidiThruConnection.Create ("com.xamarin.midi", cnnParams, out err)) {
				ClassicAssert.IsTrue (err == MidiError.Ok, "midi connection error");
				ClassicAssert.IsNotNull (connection, "midi connection should not be null");
			}
		}
		...
		public void GetSetParamsTest ()
		{
			...
			using (var connection = MidiThruConnection.Create ("com.xamarin.midi", cnnParams, out err)) {
				...
				ClassicAssert.IsTrue (gotParams.Controls.Length == cnnParams.Controls.Length, "midi params objects should be the same amount");
				for (var i = 0; i < gotParams.Controls.Length; i++) {
					ClassicAssert.AreEqual (cnnParams.Controls [i].ControlNumber, gotParams.Controls [i].ControlNumber, $"ControlNumber [{i}]");
					ClassicAssert.AreEqual (cnnParams.Controls [i].ControlType, gotParams.Controls [i].ControlType, $"ControlType [{i}]");
					ClassicAssert.AreEqual (cnnParams.Controls [i].Param, gotParams.Controls [i].Param, $"Param [{i}]");
					ClassicAssert.AreEqual (cnnParams.Controls [i].RemappedControlType, gotParams.Controls [i].RemappedControlType, $"RemappedControlType [{i}]");
					ClassicAssert.AreEqual (cnnParams.Controls [i].Transform, gotParams.Controls [i].Transform, $"Transform [{i}]");

				}
				ClassicAssert.IsTrue (gotParams.Maps.Length == cnnParams.Maps.Length, "midi params objects should be the same amount");
				...
				ClassicAssert.IsTrue (err == MidiError.Ok, "midi connection error");
				ClassicAssert.IsTrue (gotParams.FilterOutBeatClock, "FilterOutBeatClock should be true");
				ClassicAssert.IsFalse (gotParams.FilterOutAllControls, "FilterOutAllControls should be false");
				ClassicAssert.IsTrue (gotParams.HighNote == 5, "HighNote should be 5");
			}
		}
		...
		public void FindTest ()
		{
			...
			using (var connection2 = MidiThruConnection.Create (ownerId, cnnParams2)) {
				...
				ClassicAssert.IsTrue (err == MidiError.Ok, "midi connection error");
				ClassicAssert.NotNull (connections, "connections should not be null");
