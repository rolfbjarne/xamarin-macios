...
namespace MonoTouchFixtures.Security {
	...
	public class KeyTest {
			...
			try {
				using (var record = RecordTest.CreateSecRecord (SecKind.Key)) {
					...
					if (TestRuntime.CheckXcodeVersion (8, 0)) {
						ClassicAssert.True (public_key.IsAlgorithmSupported (SecKeyOperationType.Encrypt, SecKeyAlgorithm.RsaEncryptionPkcs1), "public/IsAlgorithmSupported/Encrypt");

#if MONOMAC
						ClassicAssert.True (public_key.IsAlgorithmSupported (SecKeyOperationType.Decrypt, SecKeyAlgorithm.RsaEncryptionPkcs1), "public/IsAlgorithmSupported/Decrypt");
						...
						using (var pub = public_key.GetPublicKey ()) {
							...
							ClassicAssert.False (pub.Handle == public_key.Handle, "public/GetPublicKey");
						}
#endif
						...
						using (var data = public_key.GetExternalRepresentation (out error)) {
							ClassicAssert.Null (error, "public/error-1");
							ClassicAssert.NotNull (data, "public/GetExternalRepresentation");

							using (var key = SecKey.Create (data, SecKeyType.RSA, SecKeyClass.Public, MinRsaKeySize, null, out error)) {
								ClassicAssert.Null (error, "public/Create/error-1");
							}
						}
					}
					...
					byte [] result;
					if (TestRuntime.CheckXcodeVersion (8, 0)) {
						ClassicAssert.False (private_key.IsAlgorithmSupported (SecKeyOperationType.Encrypt, SecKeyAlgorithm.RsaEncryptionPkcs1), "private/IsAlgorithmSupported/Encrypt");
						ClassicAssert.True (private_key.IsAlgorithmSupported (SecKeyOperationType.Decrypt, SecKeyAlgorithm.RsaEncryptionPkcs1), "private/IsAlgorithmSupported/Decrypt");
						...
						using (var data2 = private_key.GetExternalRepresentation (out error)) {
							ClassicAssert.Null (error, "private/error-1");
							ClassicAssert.NotNull (data2, "private/GetExternalRepresentation");

							using (var key = SecKey.Create (data2, SecKeyType.RSA, SecKeyClass.Private, MinRsaKeySize, null, out error)) {
								ClassicAssert.Null (error, "private/Create/error-1");
							}
						}
					}
					public_key.Dispose ();
		...
		public void RoundtripRSA1024OAEP ()
		{
			...
			try {
				using (var record = RecordTest.CreateSecRecord (SecKind.Key)) {
					...
					byte [] cipher;
					if (TestRuntime.CheckXcodeVersion (8, 0)) {
						ClassicAssert.True (public_key.IsAlgorithmSupported (SecKeyOperationType.Encrypt, SecKeyAlgorithm.RsaEncryptionOaepSha1), "public/IsAlgorithmSupported/Encrypt");
						// I would have expect false
#if MONOMAC
						ClassicAssert.True (public_key.IsAlgorithmSupported (SecKeyOperationType.Decrypt, SecKeyAlgorithm.RsaEncryptionOaepSha1), "public/IsAlgorithmSupported/Decrypt");
#else
						ClassicAssert.True (public_key.IsAlgorithmSupported (SecKeyOperationType.Decrypt, SecKeyAlgorithm.RsaEncryptionOaepSha1), "public/IsAlgorithmSupported/Decrypt");
#endif
					}
					...
					byte [] result;
					if (TestRuntime.CheckXcodeVersion (8, 0)) {
						ClassicAssert.False (private_key.IsAlgorithmSupported (SecKeyOperationType.Encrypt, SecKeyAlgorithm.RsaEncryptionOaepSha1), "private/IsAlgorithmSupported/Encrypt");
						ClassicAssert.True (private_key.IsAlgorithmSupported (SecKeyOperationType.Decrypt, SecKeyAlgorithm.RsaEncryptionOaepSha1), "private/IsAlgorithmSupported/Decrypt");
					}
		...
		public void BenchmarkManaged4096 ()
		{
			...
			ClassicAssert.IsNotNull (rsa.ExportParameters (true), "ExportParameters"); // that will provoke the key generation
		...
		public void RSA ()
		{
			...
			using (var key = SecKey.CreateRandomKey (SecKeyType.RSA, MinRsaKeySize, null, out error)) {
				ClassicAssert.Null (error, "RSA/error");
				...
				using (var data = NSData.FromArray (new byte [] { 1, 2, 3 })) {
						...
						using (var pub = key.GetPublicKey ()) {
							...
							using (var cipher = pub.CreateEncryptedData (SecKeyAlgorithm.RsaEncryptionPkcs1, data, out error)) {
								...
								if (badDecrypt) {
									...
									ClassicAssert.Null (key.CreateDecryptedData (SecKeyAlgorithm.RsaEncryptionPkcs1, data, out error), "bad data");
									ClassicAssert.NotNull (error, "bad decrypt");
								}
							}
						}
					}
					...
					using (var sig = key.CreateSignature (SecKeyAlgorithm.EcdsaSignatureRfc4754, data, out error)) {
						ClassicAssert.NotNull (error, "wrong key type");
					}
				}
			}
		}
		...
		public void EC ()
		{
			...
			using (var key = SecKey.CreateRandomKey (SecKeyType.EC, 384, null, out error)) {
				ClassicAssert.Null (error, "EC/error");
				using (var data = NSData.FromArray (new byte [] { 1, 2, 3 })) {
					using (var sig = key.CreateSignature (SecKeyAlgorithm.EcdsaSignatureRfc4754, data, out error)) {
						ClassicAssert.Null (error, "Sign/error");
						...
						using (var pub = key.GetPublicKey ()) {
							...
							using (var cipher = pub.CreateEncryptedData (SecKeyAlgorithm.EciesEncryptionCofactorX963Sha1AesGcm, data, out error)) {
								...
								ClassicAssert.Null (key.CreateDecryptedData (SecKeyAlgorithm.EciesEncryptionCofactorX963Sha1AesGcm, data, out error), "bad data");
								ClassicAssert.NotNull (error, "bad decrypt");
							}
						}
					}
		...
		public void ECSecPrimeRandom ()
		{
			...
			using (var key = SecKey.CreateRandomKey (SecKeyType.ECSecPrimeRandom, 384, null, out error)) {
				ClassicAssert.Null (error, "ECSecPrimeRandom/error");
				...
				using (var ex = key.GetKeyExchangeResult (SecKeyAlgorithm.EcdhKeyExchangeStandardX963Sha512, pub, p.Dictionary, out error)) {
					ClassicAssert.Null (error, "GetKeyExchangeResult/error");
