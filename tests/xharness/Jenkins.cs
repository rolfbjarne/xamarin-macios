	...
	public class Jenkins
	{
		...
		async Task PopulateTasksAsync ()
		{
			...
			foreach (var project in Harness.MacTestProjects) {
				...
				switch (project.TargetFrameworkFlavor) {
				...
				case MacFlavors.System:
					...
					break;
				default:
					throw new NotImplementedException (project.TargetFrameworkFlavor.ToString ());
				}
				foreach (var config in configurations) {
					...
					if (project.IsNUnitProject) {
						...
						exec.Variation = project.TargetFrameworkFlavor.ToString ();
						execs = new [] { exec };
					} else {
