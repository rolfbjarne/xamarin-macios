	...
	public class ManagedRegistrarLookupTablesStep : ConfigurationAwareStep {
		...
		void CollectRegistrarType (AssemblyTrampolineInfo info, AssemblyDefinition currentAssembly)
		{
			...
			if (registrarType is null) {
				Console.WriteLine ($"TODO: no __Registrar__ was found in the assembly {currentAssembly.Name.Name} after the PrepareAssemblies step, but one was found. This might be a sign that the PrepareAssemblies step didn't run, or didn't run correctly.");
				return;
			}
