...
public static class ProcessHelper
{
	...
	{
		AssertRunProcess (filename, arguments, timeout, workingDirectory, null, message);
	}
	...
	public static void BuildSolution (string solution, string platform, string configuration, Dictionary<string, string> environment_variables, string target = "")
	{
		try {
			...
			if (!string.IsNullOrEmpty (target))
				sb.Add ($"/t:{target}");
			AssertRunProcess ("msbuild", sb.ToArray (), TimeSpan.FromMinutes (5), Configuration.SampleRootDirectory, "build");
		} finally {
	...
	{
		try {
			AssertRunProcess ("make", string.IsNullOrEmpty (target) ? new string [0] : new string [] { target }, TimeSpan.FromMinutes (5), Path.GetDirectoryName (makefile), "build");
		} finally {
			// Clean up after us, since building for device needs a lot of space.
			// Ignore any failures (since failures here doesn't mean the test failed).
