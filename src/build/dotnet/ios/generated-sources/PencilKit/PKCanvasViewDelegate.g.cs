...
namespace PencilKit {
	...
	[ProtocolMember (IsRequired = false, IsProperty = false, IsStatic = false, Name = "DidRefineStrokes", Selector = "canvasView:didRefineStrokes:withNewStrokes:", ParameterType = new Type [] { typeof (PencilKit.PKCanvasView), typeof (PencilKit.PKStroke[]), typeof (PencilKit.PKStroke[]) }, ParameterByRef = new bool [] { false, false, false })]
		...
		UIKit.IUIScrollViewDelegate
	{
		...
		{
			_DidRefineStrokes (this, canvasView, strokes, newStrokes);
		}
		[SupportedOSPlatform ("ios18.1")]
		[UnsupportedOSPlatform ("maccatalyst")]
		[UnsupportedOSPlatform ("macos")]
		[BindingImpl (BindingImplOptions.GeneratedCode | BindingImplOptions.Optimizable)]
		internal static void _DidRefineStrokes (IPKCanvasViewDelegate This, PKCanvasView canvasView, PKStroke[] strokes, PKStroke[] newStrokes)
		{
			var canvasView__handle__ = canvasView!.GetNonNullHandle (nameof (canvasView));
			if (strokes is null)
				ObjCRuntime.ThrowHelper.ThrowArgumentNullException (nameof (strokes));
			if (newStrokes is null)
				ObjCRuntime.ThrowHelper.ThrowArgumentNullException (nameof (newStrokes));
			using var nsa_strokes = NSArray.FromNSObjects (strokes);
			using var nsa_newStrokes = NSArray.FromNSObjects (newStrokes);
			global::ObjCRuntime.Messaging.void_objc_msgSend_NativeHandle_NativeHandle_NativeHandle (This.Handle, Selector.GetHandle ("canvasView:didRefineStrokes:withNewStrokes:"), canvasView__handle__, nsa_strokes.Handle, nsa_newStrokes.Handle);
			GC.KeepAlive (This);
			GC.KeepAlive (canvasView);
		}
		...
		[DynamicDependencyAttribute ("DidRefineStrokes(PencilKit.PKCanvasView,PencilKit.PKStroke[],PencilKit.PKStroke[])")]
...
namespace PencilKit {
	...
	/// </summary>
	[Protocol()]
	[Register("Microsoft_iOS__PencilKit_PKCanvasViewDelegate", false)]
	[Model]
	[SupportedOSPlatform ("ios13.0")]
	[UnsupportedOSPlatform ("macos")]
	[SupportedOSPlatform ("maccatalyst")]
