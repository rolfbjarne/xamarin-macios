 ...
 return vec4 (s.rgb * darken, s.a);
		...
		class MyCustomFilter : CIFilter {
			public MyCustomFilter (CustomerFilterType type)
			{
				...
				ClassicAssert.IsNotNull (kernel, $"Kernel: {Type}");
			}
...
#endif
						ClassicAssert.IsNotNull (finalImg, "CIKernel_BasicTest should not be null");
						ClassicAssert.IsTrue (filter.CallbackHit, "CIKernel_BasicTest callback must be hit");
						if (filter.IsColorKernel)
							ClassicAssert.IsTrue (filter.kernel is CIColorKernel, "CIKernel_BasicTest we disagree that it is a color kernel");
						else
							ClassicAssert.IsTrue (filter.kernel is CIWarpKernel, "CIKernel_BasicTest we disagree that it is a warp kernel");
					}
				} catch (Exception ex2) {
					ex = ex2;
				}
		...
		public void CIKernel_TestFromPrograms ()
		{
			...
			ClassicAssert.AreEqual (4, kernels.Length, "CIKernel_TestFromPrograms did not get back the right number of programs");
			foreach (CIKernel kernel in kernels) {
				ClassicAssert.IsTrue (kernel is CIColorKernel || kernel is CIWarpKernel, "CIKernel_TestFromPrograms is neither type of kernel?");
			}
		}
	}
}
