...
namespace MonoTouchFixtures.Foundation {
	...
	public class NSFileManagerTest {
		...
		[Test]
		public void GetUserNameTest () => ClassicAssert.IsNotNull (NSFileManager.UserName);

		[Test]
		public void GetUserFullNameTest () => ClassicAssert.IsNotNull (NSFileManager.FullUserName); // cannot check the value since it depends on the enviroment

		[Test]
		public void GetHomeDirectoryTest () => ClassicAssert.IsNotNull (NSFileManager.HomeDirectory); // cannot check the value since it depends on the enviroment

		[Test]
		public void GetHomeDirectoryForUserTest () => ClassicAssert.AreEqual (NSFileManager.HomeDirectory, NSFileManager.GetHomeDirectory (NSFileManager.UserName));

		[Test]
		public void TemporaryDirectoryTest () => ClassicAssert.IsNotNull (NSFileManager.TemporaryDirectory); // cannot check the value since it depends on the enviroment
		...
		{
			ClassicAssert.False (NSFileManager.GetSkipBackupAttribute (NSBundle.MainBundle.ExecutableUrl.ToString ()), "MainBundle");
			...
			try {
				...
				ClassicAssert.False (NSFileManager.GetSkipBackupAttribute (filename), "DoNotBackupMe-0");

				NSFileManager.SetSkipBackupAttribute (filename, true);

				NSError error;
				ClassicAssert.True (NSFileManager.GetSkipBackupAttribute (filename, out error), "DoNotBackupMe-1");
				ClassicAssert.Null (error, "error-1");

				error = NSFileManager.SetSkipBackupAttribute (filename, false);
				ClassicAssert.False (NSFileManager.GetSkipBackupAttribute (filename), "DoNotBackupMe-2");
				ClassicAssert.Null (error, "error-2");
			} finally {
		...
		public void DefaultManager ()
		{
			...
			ClassicAssert.NotNull (NSFileManager.DefaultManager, "DefaultManager");
		}
