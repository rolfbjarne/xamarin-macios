...
namespace MonoTouchFixtures.Simd
{
	...
	public class MatrixFloat3x3Test
	{
		...
		// between Matrix3 and MatrixFloat3x3 (in particular Invert, which is 
	...
	[TestFixture]
	[Preserve (AllMembers = true)]
	public class MatrixFloat3x3PerfTest
	{
		int Multiplier {
			get {
		...
		public void Determinant ()
		{
			...
			MatrixFloat3x3 input_simd = (MatrixFloat3x3) input;
		...
		public void Ctor_Elements ()
		{
			...
			var watch = Stopwatch.StartNew ();
			for (int i = 0; i < iterations; i++)
				input = new Matrix3 (1, 2, 3, 1, 2, 3, 1, 2, 3);
			watch.Stop ();

			var watch_simd = Stopwatch.StartNew ();
			MatrixFloat3x3 input_simd;
			for (int i = 0; i < iterations; i++)
				input_simd = new MatrixFloat3x3 (1, 2, 3, 1, 2, 3, 1, 2, 3);
			watch_simd.Stop ();
		...
		public void Multiply_ByRef ()
		{
			...
			var watch = Stopwatch.StartNew ();
			for (int i = 0; i < iterations; i++)
				Matrix3.Multiply (ref input_a, ref input_b, out result);
			watch.Stop ();

			MatrixFloat3x3 input_simd_a = (MatrixFloat3x3) input_a;
			MatrixFloat3x3 input_simd_b = (MatrixFloat3x3) input_b;
			MatrixFloat3x3 result_simd;
			var watch_simd = Stopwatch.StartNew ();
			for (int i = 0; i < iterations; i++)
				MatrixFloat3x3.Multiply (ref input_simd_a, ref input_simd_b, out result_simd);
			watch_simd.Stop ();
		...
		{
			return MatrixFloat3x3Test.GetTestMatrix ();
		}
	}

	static class Matrix3Extension
	{
		public static Vector3 GetRow0 (this Matrix3 m)
		{
			return new Vector3 (m.R0C0, m.R0C1, m.R0C2);
		}

		public static Vector3 GetRow1 (this Matrix3 m)
		{
			return new Vector3 (m.R1C0, m.R1C1, m.R1C2);
		}

		public static Vector3 GetRow2 (this Matrix3 m)
		{
			return new Vector3 (m.R2C0, m.R2C1, m.R2C2);
		}

		public static Vector3 GetColumn0 (this Matrix3 m)
		{
			return new Vector3 (m.R0C0, m.R1C0, m.R2C0);
		}

		public static Vector3 GetColumn1 (this Matrix3 m)
		{
			return new Vector3 (m.R0C1, m.R1C1, m.R2C1);
		}

		public static Vector3 GetColumn2 (this Matrix3 m)
		{
			return new Vector3 (m.R0C2, m.R1C2, m.R2C2);
		}
	}
}
