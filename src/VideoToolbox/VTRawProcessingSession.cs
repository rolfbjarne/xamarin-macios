...
namespace VideoToolbox {
	/// <summary>This delegate is called when available parameters or parameter values change without an explicit call to <see cref="VTRawProcessingSession.SetProcessingParameters" />.</summary>
	...
	public class VTRawProcessingSession : NativeObject {
		...
		/// <param name="error">An error code if the operation was unsuccessful, otherwise <see cref="CVStatus.Ok" />.</param>
		...
		/// <remarks>This will also call <see cref="Invalidate" />.</summary>
		...
		static void VTRawProcessingParameterChangeHandlerCallback (IntPtr block, IntPtr newParameters)
		{
			...
			if (del is not null) {
				var newParams = NSArray.FromHandle<NSObject> (newParameters);
				return del (newParams);
			}
		}
#endif
		...
		static void VTRawProcessingOutputHandlerCallback (IntPtr block, VTStatus status, IntPtr processedPixelBuffer)
		{
			...
			if (del is not null) {
				var pb = Runtime.GetINativeObject<CVPixelBuffer> (handle, owns: false);
				return del (status, pb);
			}
		}
#endif
		...
		public NSDictionary[]? CopyProcessingParameters (out VTStatus status)
		{
			...
			if (status == VTStatus.Ok && handle != IntPtr.Zero) {
				var rv = NSArray.FromHandle<NSDictionary> (handle);
				rv.DangerousRelease (); // owns: true
				return rv;
			}
			return null;
		}
		...
		public VTRawProcessingParameter []? ProcessingParameters {
			get {
				var dictionaries = CopyProcessingParameters (out var _);
				if (dictionaries is null)
					return null;
				var rv = new VTRawProcessingParameter [dictionaries.Length];
				for (var i = 0; i < rv.Length; i++)
					rv [i] = new VTRawProcessingParameter (dictionaries [i]);
		...
		/// <param name="processingParameter">The parameters to set.</param>
		/// <returns>An error code if the operation was unsuccessful, otherwise <see cref="CVStatus.Ok" />.</returns>
		public VStatus SetProcessingParameters (NSDictionary processingParameters)
		{
		...
		/// <param name="processingParameter">The parameters to set.</param>
		/// <returns>An error code if the operation was unsuccessful, otherwise <see cref="CVStatus.Ok" />.</returns>
		public VStatus SetProcessingParameters (VTRawProcessingParameter processingParameters)
		{
			SetProcessingParameters (processingParameters?.Dictionary);
		}
	}
}
#endif // MONOMAC
