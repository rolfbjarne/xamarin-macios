	...
	public partial class Target {
		...
		public void Initialize (bool show_warnings)
		{
			...
			if (root == null) {
				...
				throw ErrorHelper.CreateError (2018, "Can not load the root assembly '{0}'.", App.RootAssembly);
			}
			...
			foreach (var reference in App.References) {
				...
				if (ad.MainModule.FileName == root.MainModule.FileName) {
					// If we asked the manifest resolver for assembly X and got back the root assembly, it means the requested assembly conflicts with the root assembly.
		...
		public void Compile ()
		{
			...
			if (App.Registrar == RegistrarMode.Dynamic && App.IsSimulatorBuild && App.LinkMode == LinkMode.None) {
				...
				linker_flags.AddLinkWith (Path.Combine (Driver.GetMonoTouchLibDirectory (App), library));
			}
