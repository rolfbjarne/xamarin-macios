...
namespace ObjCRuntime
{
	class Stret
	{
		...
		public static bool ArmNeedStret (Type returnType, Generator generator)
		{
			...
			if (!t.IsValueType || t.IsEnum || IsBasicType (t))
				return false;
		...
		{
			return IsBasicType (type, true /* doesn't matter */, out var _);
		}

		static bool IsBasicType (Type type, bool is_64_bits, out int type_size)
		{
			type_size = 0;
		...
		static void GetValueTypeSize (Type original_type, Type type, List<Type> field_types, bool is_64_bits, ref int size, ref int max_element_size, Generator generator)
		{
			...
			if (IsBasicType (type, is_64_bits, out var type_size) && type_size > 0) {
