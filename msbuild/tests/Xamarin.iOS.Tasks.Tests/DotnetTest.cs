...
namespace Xamarin.iOS.Tasks {
	...
	public class DotnetTest : ProjectTest {
		...
		public void CompareBuilds (string project)
		{
			...
			var net461_files = Directory.GetFiles (net461_bundle, "*.*", SearchOption.AllDirectories).Select ((v) => v.Substring (net461_bundle.Length));
			var dotnet_files = Directory.GetFiles (dotnet_bundle, "*.*", SearchOption.AllDirectories).Select ((v) => v.Substring (dotnet_bundle.Length));
			...
			foreach (var file in dotnet_files) {
				...
				if (dotnet_size == net461_size)
					continue;
				Console.WriteLine ($"{file}: {net461_size} bytes -> {dotnet_size} bytes");
			}
			Console.WriteLine ("Size comparison complete");
		}
	}
}
