...
namespace MonoTouchFixtures.Foundation {
	...
	public class NSSetTest {
		...
		public void SetCtors ()
		{
			...
			var s = new NSSet (1);
			Assert.That ((nuint) 1, Is.EqualTo (s.Count));
			s = new NSSet (1, 2, 3);
			Assert.That ((nuint) 3, Is.EqualTo (s.Count));
			...
			Assert.That ((nuint) 5, Is.EqualTo (s.Count));
		...
		public void OperatorPlus ()
		{
			...
			var sum = one + two;
			Assert.That ((nuint) 6, Is.EqualTo (sum.Count));
			...
			Assert.That ((nuint) 11, Is.EqualTo (sum.Count));
			sum = new NSSet (objs) + new NSSet (objs);
			Assert.That ((nuint) 5, Is.EqualTo (sum.Count));

			Assert.That ((nuint) 3, Is.EqualTo ((one + one).Count));
			var sub = one - one;
			Assert.That ((nuint) 0, Is.EqualTo (sub.Count));

			var three = new NSSet (1, 2, 3, 4, 5, 6);
			var subt = three - two;
			Assert.That ((nuint) 3, Is.EqualTo (subt.Count));
