...
namespace MonoTouchFixtures.AudioToolbox {
	...
	public class AudioConverterTest {
		...
		public void Properties ()
		{
			...
			ClassicAssert.AreEqual (AudioConverterError.None, createResult, $"AudioConverterCreate ({srcFormat} -> {dstFormat}): {createResult}");
		...
		void Convert (string sourceFilePath, string destinationFilePath, AudioFormatType outputFormatType, int? sampleRate = null, AudioConverterOptions? options = null, bool withPacketDependencies = false)
		{
			...
			if (outputFormatType == AudioFormatType.LinearPCM) {
				...
				ClassicAssert.AreEqual (AudioFormatError.None, afe, $"GetFormatInfo: {name}");
			...
			ClassicAssert.AreEqual (AudioConverterError.None, ce, $"AudioConverterCreate : {name}\n\tSource format: {srcFormat}\n\tDestination format: {dstFormat})");
			...
			while (true) {
				...
				ClassicAssert.AreEqual (AudioConverterError.None, fe, $"FillComplexBuffer: {name}");
				...
				ClassicAssert.AreEqual (AudioFileError.Success, we, $"WritePackets: {name}");
