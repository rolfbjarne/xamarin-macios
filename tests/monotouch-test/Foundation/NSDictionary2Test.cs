...
namespace MonoTouchFixtures.Foundation {
	...
	public class NSDictionary2Test {
		...
		public void Ctor_Arrays ()
		{
			...
			ClassicAssert.AreEqual ((string) (j [(NSString) "first-k"]), "first", "lookup1");
			ClassicAssert.AreEqual ((string) (j [(NSString) "second-k"]), "second", "lookup2");
		}
		...
		public void FromObjectsAndKeysGenericTest_NullValue ()
		{
			...
			var dict = NSDictionary<NSString, NSNumber>.FromObjectsAndKeys (values, keys, values.Length);
			ClassicAssert.AreEqual (dict.Count, (nuint) 3, "count");
			ClassicAssert.AreEqual (dict [keys [0]], values [0], "key lookup 0");
			var baseDict = (NSDictionary) dict;
			var rawValue = baseDict.ObjectForKey (keys [1]);
			ClassicAssert.IsInstanceOf<NSNull> (rawValue, "Null value");
			ClassicAssert.AreEqual (dict [keys [2]], values [2], "key lookup 2");
		}
		...
		public void FromObjectsAndKeysGenericTest_NSObjects_NullValue ()
		{
			...
			var dict = NSDictionary<NSString, NSNumber>.FromObjectsAndKeys (values, keys, values.Length);
			ClassicAssert.AreEqual (dict.Count, (nuint) 3, "count");
			ClassicAssert.AreEqual (1, dict [(NSString) keys [0]].ByteValue, "key lookup 0");
			var baseDict = (NSDictionary) dict;
			var rawValue = baseDict.ObjectForKey ((NSString) keys [1]);
			ClassicAssert.IsInstanceOf<NSNull> (rawValue, "Null value");
			ClassicAssert.AreEqual (42, dict [(NSString) keys [2]].Int32Value, "key lookup 2");
		}
		...
		public void ReadOnlyDictionaryTest ()
		{
			...
			ClassicAssert.IsTrue (idict.IsReadOnly, "IsReadOnly");
		...
		public void IDictionary2Test ()
		{
			...
			// IsReadOnly
			ClassicAssert.IsTrue (dict.IsReadOnly, "IsReadOnly");
