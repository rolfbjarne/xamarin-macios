...
namespace MonoTouchFixtures.AudioToolbox {
	...
	public class AudioConverterTest {
		...
		public void CreateWithOptions ()
		{
			...
			Convert (sourcePath, output1, AudioFormatType.AppleLossless, AudioConverterOptions.None);

			// Convert converted output
			var output2 = Path.Combine (paths [0], "outputOptions2.wav");
			Convert (output1, output2, AudioFormatType.LinearPCM, AudioConverterOptions.None);
		}
		...
		void Convert (string sourceFilePath, string destinationFilePath, AudioFormatType outputFormatType, int? sampleRate = null, AudioConverterOptions? options)
		{
			...
			using AudioConverter? converter = null;
			if (options.HasValue) {
				converter = AudioConverter.Create (srcFormat, dstFormat, options.Value, out var ce);
			} else {
				converter = AudioConverter.Create (srcFormat, dstFormat, out var ce);
			}
