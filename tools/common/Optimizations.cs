		...
		public void Initialize (Application app, out List<ProductException> messages)
		{
			...
			for (int i = 0; i < values.Length; i++) {
				...
				case Opt.StaticBlockToDelegateLookup:
					if (app.Registrar != RegistrarMode.Static && app.Registrar != RegistrarMode.ManagedStatic && app.Registrar != RegistrarMode.TrimmableStatic) {
			...
			if (!OptimizeBlockLiteralSetupBlock.HasValue) {
				OptimizeBlockLiteralSetupBlock = app.Registrar == RegistrarMode.Static || app.Registrar == RegistrarMode.ManagedStatic || app.Registrar == RegistrarMode.TrimmableStatic;
			}
			...
			if (!RegisterProtocols.HasValue) {
				if (app.Platform != ApplePlatform.MacOSX || app.XamarinRuntime == XamarinRuntime.NativeAOT) {
					RegisterProtocols = (app.Registrar == RegistrarMode.Static || app.Registrar == RegistrarMode.ManagedStatic || app.Registrar == RegistrarMode.TrimmableStatic);
				} else {
					RegisterProtocols = false;
				}
			} else if (app.Registrar != RegistrarMode.Static && app.Registrar != RegistrarMode.ManagedStatic && app.Registrar != RegistrarMode.TrimmableStatic && RegisterProtocols == true) {
			...
			if (!RemoveDynamicRegistrar.HasValue) {
				if (app.XamarinRuntime == XamarinRuntime.NativeAOT) {
					...
					RemoveDynamicRegistrar = false;
				} else if ((app.Registrar != RegistrarMode.Static && app.Registrar != RegistrarMode.ManagedStatic && app.Registrar != RegistrarMode.TrimmableStatic) || !app.AreAnyAssembliesTrimmed) {
