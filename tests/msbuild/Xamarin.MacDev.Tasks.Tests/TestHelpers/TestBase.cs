	...
	public abstract class TestBase {
		...
		public void ExecuteTask (Task task, int expectedErrorCount = 0, string message = "")
		{
			...
			if (expectedErrorCount != Engine.Logger.ErrorEvents.Count) {
				ClassicAssert.AreEqual (expectedErrorCount, Engine.Logger.ErrorEvents.Count, $"#RunTask-ErrorCount{(string.IsNullOrEmpty (message) ? "" : $" ({message})")}" + messages);
			}
			ClassicAssert.AreEqual (expectedErrorCount == 0, rv, $"Failure{(string.IsNullOrEmpty (message) ? "" : $" ({message})")}" + messages);
		}
