		...
		public override bool Execute ()
		{
			...
			foreach (var item in list.ToArray ()) { // iterate over a copy of the list, because we might modify the original list
				...
				case PublishFolderType.CompressedAppleFramework:
					...
					virtualProjectPath = string.Empty; // the name of the compressed file doesn't matter, we handle every *.[xc]framework inside it like a framework.
					break;
				...
				case PublishFolderType.CompressedPlugIns:
					relativePath = PlugInsDirectory;
					virtualProjectPath = string.Empty; // the name of the compressed file doesn't matter, we copy everything in the zip file into the PlugIns directory as-is
					break;
