		...
		public void TlsDefaults ()
		{
			...
			using (var queue = new DispatchQueue (GetType ().FullName)) {
				...
				using (var m = connection.GetProtocolMetadata<NWTlsMetadata> (NWProtocolDefinition.CreateTlsDefinition ())) {
					...
					ClassicAssert.False (s.EarlyDataAccepted, "EarlyDataAccepted");
					ClassicAssert.Null (s.NegotiatedProtocol, "NegotiatedProtocol");
					...
					ClassicAssert.True (SecProtocolMetadata.ChallengeParametersAreEqual (s, s), "ChallengeParametersAreEqual");
					ClassicAssert.True (SecProtocolMetadata.PeersAreEqual (s, s), "PeersAreEqual");
					...
					if (TestRuntime.CheckXcodeVersion (11, 0)) {
						...
						ClassicAssert.False (s.AccessPreSharedKeys ((psk, pskId) => { }), "AccessPreSharedKeys");
					}
				}
