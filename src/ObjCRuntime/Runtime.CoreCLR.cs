...
namespace ObjCRuntime {
	...
	public partial class Runtime {
		...
		static unsafe MonoObject* InvokeMethod (MonoObject* methodobj, IntPtr instance_gchandle, IntPtr native_parameters)
		{
			var rv = InvokeMethod ((MethodBase) GetMonoObjectTarget (methodobj), instance_gchandle, native_parameters);
			if (rv == null)
				return null;
		...
		static object InvokeMethod (MethodBase method, IntPtr instance_gchandle, IntPtr native_parameters)
		{
			...
			log_coreclr ($"InvokeMethod ({method.DeclaringType.FullName}::{method}, 0x{instance_gchandle.ToString ("x")} => {instance}, 0x{native_parameters.ToString ("x")})");
			unsafe {
			...
			log_coreclr ($"InvokeMethod ({method.DeclaringType.FullName}::{method}, 0x{instance_gchandle.ToString ("x")} => {instance}, 0x{native_parameters.ToString ("x")}) INVOKING");
			...
			log_coreclr ($"InvokeMethod ({method}, 0x{instance_gchandle.ToString ("x")} => {instance}, 0x{native_parameters.ToString ("x")}) Ref parameters: {byrefParameterCount} Return value of type {rv?.GetType ()} {(rv is bool ? rv : null)}");
