...
#pragma warning restore CS8618
		...
		static unsafe Runtime.MTRegistrationMap* FindMap (Assembly assembly)
		{
			...
			return FindMap (assembly.GetName ().Name!);
		}
		...
		{
			return FindClass (FindMap (type.Assembly), type, out is_custom_type);
		}

		// Find the given managed type in the tables generated by the static registrar.
		unsafe static IntPtr FindClass (Runtime.MTRegistrationMap* map, Type type, out bool is_custom_type)
		{
			is_custom_type = false;
...
#if LOG_TYPELOAD
			Console.WriteLine ($"FindType (0x{@class:X} = {Marshal.PtrToStringAuto (class_getName (@class))}) => mapIndex: {mapIndex} is custom: {is_custom_type} (token reference: 0x{type_reference:X}).");
#endif
			type = ResolveTypeTokenReference (map, type_reference);
		...
		internal unsafe static MemberInfo? ResolveFullTokenReference (Runtime.MTRegistrationMap *map, uint token_reference)
		{
			var idx = (int) (token_reference >> 1);
			var entry = map->full_token_references [idx];
			var assembly_name = map->assemblies [entry.assembly_index].name;
			var module_token = entry.module_token;
			var token = entry.token;
		...
		internal unsafe static Type? ResolveTypeTokenReference (Runtime.MTRegistrationMap* map, uint token_reference)
		{
			var member = ResolveTokenReference (map, token_reference, 0x02000000 /* TypeDef */);
			if (member is null)
				return null;
		...
		internal unsafe static MethodBase? ResolveMethodTokenReference (Runtime.MTRegistrationMap* map, uint token_reference)
		{
			var member = ResolveTokenReference (map, token_reference, 0x06000000 /* Method */);
			if (member is null)
				return null;
		...
		{
			return ResolveTokenReference (Runtime.options->RegistrationMap, token_reference, implicit_token_type);
		}

		unsafe static MemberInfo? ResolveTokenReference (Runtime.MTRegistrationMap *map, uint token_reference, uint implicit_token_type)
		{
			if ((token_reference & 0x1) == 0x1)
				return ResolveFullTokenReference (map, token_reference);
		...
		internal unsafe static uint GetTokenReference (Runtime.MTRegistrationMap* map, Type type, bool throw_exception = true)
		{
			if (type.IsGenericType)
				type = type.GetGenericTypeDefinition ();
