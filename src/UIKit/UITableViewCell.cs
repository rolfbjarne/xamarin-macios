...
namespace UIKit {
	...
	public partial class UITableViewCell {
		...
		/// <remarks>
		///   <para>The reuse identifier is associated with all cells (rows) in a table view that have the same layout (irrespective of their content) and can therefore be used interchangeably. The <see cref="UITableViewSource.GetCell(UITableView,Foundation.NSIndexPath)" /> implementation calls <see cref="UITableView.DequeueReusableCell(Foundation.NSString)" /> with a specific reuse identifier string to obtain a cached cell object with AParticular layout to use as the basis for the row being constructed for viewing.</para>
