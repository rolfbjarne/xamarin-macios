...
namespace Xamarin.Bundler {
	public partial class Driver {
		...
		{
			options.Add ("warnaserror:", "A comma-separated list of warning codes that should be reported as errors.", v =>
			{
				try {
			...
			options.Add ("nowarn:", "A comma-separated list of warning codes to ignore.", v =>
			{
				try {
					if (!string.IsNullOrEmpty (v)) {
						foreach (var code in v.Split (new char [] { ',' }, StringSplitOptions.RemoveEmptyEntries))
