...
namespace MonoTouchFixtures.Compression {
	...
	public class DeflateStreamTest {
		...
		public void CheckCompressDecompress (CompressionAlgorithm algorithm)
		{
			...
			ClassicAssert.AreNotEqual (0, output.Length, "Length should be more than 0.");
			ClassicAssert.IsTrue (compare_buffers (data, output.GetBuffer (), (int) output.Length), "Buffers are not equal.");
			decompressing.Close ();
			output.Close ();
		}
		...
		public void CheckDecompress ()
		{
			...
			ClassicAssert.AreEqual ("Hello", reader.ReadLine ());
			decompressing.Close ();
		}
		...
		public void CheckGetCanSeekProp ()
		{
			...
			ClassicAssert.IsFalse (decompress.CanSeek, "#A1");
			ClassicAssert.IsTrue (backing.CanSeek, "#A2");
			decompress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanSeek, "#A3");
			ClassicAssert.IsFalse (backing.CanSeek, "#A4");
			...
			ClassicAssert.IsFalse (compress.CanSeek, "#B1");
			ClassicAssert.IsTrue (backing.CanSeek, "#B2");
			compress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanSeek, "#B3");
			ClassicAssert.IsFalse (backing.CanSeek, "#B4");
		}
		...
		public void CheckGetCanReadProp ()
		{
			...
			MemoryStream backing = new MemoryStream (compressed_data);
			DeflateStream decompress = new DeflateStream (backing, CompressionMode.Decompress, CompressionAlgorithm.Zlib);
			ClassicAssert.IsTrue (decompress.CanRead, "#A1");
			ClassicAssert.IsTrue (backing.CanRead, "#A2");
			decompress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanRead, "#A3");
			ClassicAssert.IsFalse (backing.CanRead, "#A4");

			backing = new MemoryStream ();
			DeflateStream compress = new DeflateStream (backing, CompressionMode.Compress, CompressionAlgorithm.Zlib);
			ClassicAssert.IsFalse (compress.CanRead, "#B1");
			ClassicAssert.IsTrue (backing.CanRead, "#B2");
			compress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanRead, "#B3");
			ClassicAssert.IsFalse (backing.CanRead, "#B4");
		}
		...
		public void CheckGetCanWriteProp ()
		{
			...
			MemoryStream backing = new MemoryStream ();
			DeflateStream decompress = new DeflateStream (backing, CompressionMode.Decompress, CompressionAlgorithm.Zlib);
			ClassicAssert.IsFalse (decompress.CanWrite, "#A1");
			ClassicAssert.IsTrue (backing.CanWrite, "#A2");
			decompress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanWrite, "#A3");
			ClassicAssert.IsFalse (backing.CanWrite, "#A4");

			backing = new MemoryStream ();
			DeflateStream compress = new DeflateStream (backing, CompressionMode.Compress, CompressionAlgorithm.Zlib);
			ClassicAssert.IsTrue (compress.CanWrite, "#B1");
			ClassicAssert.IsTrue (backing.CanWrite, "#B2");
			compress.Dispose ();
			ClassicAssert.IsFalse (decompress.CanWrite, "#B3");
			ClassicAssert.IsFalse (backing.CanWrite, "#B4");
		}
		...
		class Bug19313Stream : MemoryStream {
			...
			public override int Read (byte [] buffer, int offset, int count)
			{
				...
				ClassicAssert.IsTrue (Position < Length, "Trying to read empty stream.");
