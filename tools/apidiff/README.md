...
We use the [Microsoft.DotNet.ApiCompat.Tool][apitool] tool to detect breaking changes.

To run the tool locally against the stable assemblies, do:

```
$ make verify-no-breaking-changes
```

The tool as some limitations:

* It does not special-case `[Experimental]` APIs, so we have to add suppression rules for any breaking changes in such APIs. See also https://github.com/dotnet/sdk/issues/50637.

The suppression rules can be regenerated by executing:

```
$ make regenerate-suppression-files
```

For release branches, we run the tool in a scrict mode, where no API changes are allowed.

[apitool]: https://learn.microsoft.com/en-us/dotnet/fundamentals/apicompat/global-tool
