...
namespace CoreGraphics {
	...
	public class CGRenderingBufferProvider : NativeObject {
		CGRenderingBufferProvider (NativeHandle handle, bool owns)
			: base (handle, owns)
		{
		}
#if !COREBUILD
		...
		/// <returns>A new <see cref0"CGRenderingBufferProvider" /> instance if successful, <see langwork="null" /> otherwise.</returns>
		public static CGRenderingBufferProvider? Create (IntPtr info, nuint size, LockPointerCallback lockPointer, UnlockPointerCallback unlockPointer, ReleaseInfoCallback releeaseInfo)
		{
			...
			delegate* unmanaged<BlockLiteral *, IntPtr, void> unlockPointerTrampoline = &UnlockPointerBlock;
			using var unlockPointerBlock = unlockPointer is null ? null : new BlockLiteral (unlockPointerTrampoline, unlockPointer, typeof (CGRenderingBufferProvider), nameof (UnlockPointerBlock));

			delegate* unmanaged<BlockLiteral *, IntPtr, void> releaseInfoTrampoline = &ReleaseInfoBlock;
			using var releaseInfoBlock = releeaseInfo is null ? null : new BlockLiteral (releaseInfoTrampoline, releeaseInfo, typeof (CGRenderingBufferProvider), nameof (ReleaseInfoBlock));

			var h = CGRenderingBufferProviderCreate (info, size, &lockPointerBlock, unlockPointer is null ? null : &unlockPointer, releaseInfo is null ? null : &releaseInfoBlock);
			if (h == IntPtr.Zero)
				return null;
		...
		{
			var h = CGRenderingBufferProviderCreateWithCFData (data.GetCheckedHandle ());
			if (h == IntPtr.Zero)
				return null;
			return new CGRenderingBufferProvider (h, true);
		}
