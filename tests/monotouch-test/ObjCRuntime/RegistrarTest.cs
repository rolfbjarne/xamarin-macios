	...
	public class RegistrarTest {
		...
		public void TestBindAs ()
		{
			using (var obj = new ObjCRegistrarTest ()) {
				...
				obj.PCGPointValue = NSValue.FromCGPoint (pnt);
				...
				obj.PCGPointValue = null;
				Assert.IsNull (obj.PCGPointValue, "after property set null property");
				Assert.IsNull (obj.GetCGPointValue (), "after property set null method");
			}

			using (var obj = new BindAsTestClass ()) {
				Assert.IsNull (obj.CGPointValue, "1");
				Messaging.void_objc_msgSend_IntPtr (obj.Handle, Selector.GetHandle ("setCGPointValue:"), IntPtr.Zero);
				Assert.IsNull (obj.CGPointValue, "2");
				obj.CGPointValue = new CGPoint (1, 2);
				Messaging.void_objc_msgSend_IntPtr (obj.Handle, Selector.GetHandle ("setCGPointValue:"), IntPtr.Zero);
				Assert.IsNull (obj.CGPointValue, "3");
				var pnt = new CGPoint (3.3, 4.4);
				Messaging.void_objc_msgSend_IntPtr (obj.Handle, Selector.GetHandle ("setCGPointValue:"), NSValue.FromCGPoint (pnt).Handle);
				Assert.AreEqual (pnt, obj.CGPointValue, "4");
				Messaging.void_objc_msgSend_IntPtr (obj.Handle, Selector.GetHandle ("setCGPointValue:"), IntPtr.Zero);
				Assert.IsNull (obj.CGPointValue, "5");

				var value = Runtime.GetNSObject<NSValue> (Messaging.IntPtr_objc_msgSend (obj.Handle, Selector.GetHandle ("getCGPointValue")));
				Assert.IsNull (value, "6");
				obj.CGPointValue = pnt;
				value = Runtime.GetNSObject<NSValue> (Messaging.IntPtr_objc_msgSend (obj.Handle, Selector.GetHandle ("getCGPointValue")));
				Assert.AreEqual (pnt, value.CGPointValue, "7");
			}
		}
		...
		class BindAsTestClass : ObjCRegistrarTest
		{
			...
			public override CGPoint? GetCGPointValue ()
			{
				return CGPointValue;
			}

			public override void SetCGPointValue (CGPoint? value)
			{
				CGPointValue = value;
			}
