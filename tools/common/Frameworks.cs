		...
		get {
			if (mac_frameworks == null) {
				mac_frameworks = new Frameworks () {
					...
					{ "Intents", 10, 12 },
					{ "IOSurface", "IOSurface", new Version (10, 12), new Version (int.MaxValue, int.MaxValue) /* Not available in the simulator (the header is there, but broken) */  },
					{ "Photos", "Photos", 10,12 },
	...
	public static Frameworks GetiOSFrameworks (Application app)
	{
		if (ios_frameworks == null) {
			ios_frameworks = new Frameworks () {
				...
				{ "CloudKit", "CloudKit", 8 },
				{ "AVKit", "AVKit", 8 },
				{ "CoreAudioKit", "CoreAudioKit", new Version (app.IsSimulatorBuild ? 9 : 8, 0), new Version (int.MaxValue, int.MaxValue) /* header is not yet shipped with the iOS SDK simulator */ },
