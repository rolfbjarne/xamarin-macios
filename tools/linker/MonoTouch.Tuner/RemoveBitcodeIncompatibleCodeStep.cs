...
namespace MonoTouch.Tuner {
	...
	public class RemoveBitcodeIncompatibleCodeStep : BaseStep {
		...
		void ProcessMethod (MethodDefinition method)
		{
			...
			var anyFilterOrFaultClauses = false;
			foreach (var eh in body.ExceptionHandlers) {
				if (eh.HandlerType == ExceptionHandlerType.Fault || eh.HandlerType == ExceptionHandlerType.Filter) {
					anyFilterOrFaultClauses = true;
					ErrorHelper.Show (ErrorHelper.CreateWarning (Options.Application, 2110, method, $"The method {method.FullName} contains a '{eh.HandlerType}' exception clause, which is currently not supported when compiling for bitcode. This method will throw an exception if called."));
					break;
				}
			}
			if (!anyFilterOrFaultClauses)
				return;
