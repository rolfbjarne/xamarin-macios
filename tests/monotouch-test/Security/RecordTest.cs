...
namespace MonoTouchFixtures.Security {
	...
	public class RecordTest {
		...
		public void Identity ()
		{
			...
			using (var dict = rec.ToDictionary ()) {
				...
				if (hasIdnt)
					...
					ClassicAssert.Null (dict ["class"], "idnt");
			}
		}
		...
		public void Match ()
		{
			...
			ClassicAssert.Null (rec.MatchIssuers, "MatchIssuers");
			// we do not have a way (except the getter) to craete SecKeyChain instances
			ClassicAssert.Null (rec.MatchItemList, "MatchItemList");

			using (var data = new NSData ()) {
				rec.MatchIssuers = new NSData [] { data };
				ClassicAssert.AreSame (rec.MatchIssuers [0], data, "MatchIssuers [0]");
			}
		...
		public void DeskCase_83099_InmutableDictionary ()
		{
			...
			try {
				...
				ClassicAssert.IsTrue (test1, $"Password could not be saved to keychain. queryCode: {queryCode} addCode: {addCode} updateCode: {updateCode}");
				...
				ClassicAssert.IsTrue (test3, $"Password could not be saved to keychain. queryCode: {queryCode} addCode: {addCode} updateCode: {updateCode}");
				...
				ClassicAssert.IsTrue (test5, $"Password could not be cleared from keychain. queryCode: {queryCode} removeCode: {removeCode}");

				//TEST 6: Verify no keychain value
				var test6 = GetKeychainEntry (testServer, testUsername);
				ClassicAssert.IsNull (test6, "No password should exist here");
			} finally {
		...
		public void IdentityRecordTest ()
		{
			...
			using (var rec = CreateSecRecord (identity)) {
				...
				ClassicAssert.True (code == SecStatusCode.DuplicateItem || code == SecStatusCode.Success, "Identity added");

				var ret = rec.GetIdentity ();
				ClassicAssert.NotNull (ret, "ret is null");
		...
		public void SecRecordRecordTest ()
		{
			...
			using (var rec = CreateSecRecord (sc)) {
				ClassicAssert.NotNull (rec, "rec is null");
