...
static class C {
	...
	static string [] structs_and_stret =  {
		/* integral types */
		"c:0", "cc:4", "ccc:4",  "cccc:4", // we don't support structs with char elements.
		...
		"id:5", "di:5", // 16 bytes
		"iid:5", // 16 bytes
		// "idi:7", // 16 bytes on i386 and 24 bytes on x86_64 (due to alignment), but our generator doesn't take alignment into account, which means the generator will use the wrong objc_msgSend for x86_64 (since it thinks the structure is 16 bytes it won't use stret).
		"ddi:15", // 24 bytes
