	...
	public class MutableString {
		...
		public void TransformNoRangeEnum ()
		{
			using (var s = new CFMutableString ("Bonjour à tous!")) {
				ClassicAssert.True (s.Transform (CFStringTransform.ToXmlHex, false), "Transform-1");
				Assert.That (s.ToString (), Is.EqualTo ("Bonjour &#xE0; tous!"), "ToString-1");

				ClassicAssert.True (s.Transform (CFStringTransform.ToXmlHex, true), "Transform-2");
		...
		public void TransformRangeEnum ()
		{
			...
			using (var s = new CFMutableString ("Bonjour à tous!")) {
				ClassicAssert.True (s.Transform (ref r, CFStringTransform.ToXmlHex, false), "Transform-1");
				Assert.That (s.ToString (), Is.EqualTo ("Bonjour &#xE0; tous!"), "ToString-1");
				Assert.That (r.Length, Is.EqualTo (20), "Length-1");

				ClassicAssert.True (s.Transform (ref r, CFStringTransform.ToXmlHex, true), "Transform-2");
				Assert.That (s.ToString (), Is.EqualTo ("Bonjour à tous!"), "ToString-2");
		...
		public void TransformICU ()
		{
			using (var s = new CFMutableString ("hello world")) {
				ClassicAssert.True (s.Transform ("Title", false), "Transform-1");
				Assert.That (s.ToString (), Is.EqualTo ("Hello World"), "ToString-1");

				ClassicAssert.True (s.Transform ((NSString) "Title", true), "Transform-2");
