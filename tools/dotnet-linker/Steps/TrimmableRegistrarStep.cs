			...
			foreach (var typesInAssembly in typesByAssembly.OrderBy (v => v.Key.FullName)) {
				...
				foreach (var tr in inativeObjectTypes.OrderBy (v => v.FullName)) {
					...
					*/
					...
					createObjectMethod.AddParameter (abr.System_IntPtr);
					createObjectMethod.AddParameter (abr.System_Boolean);
				...
				foreach (var kvp in typesInAssembly.OrderBy (v => v.Key.FullName)) {
					...
					if (!objcType.IsProtocol && !objcType.IsCategory) {
						...
						getClassHandleMethod.AddParameter (abr.System_Boolean.MakeByReferenceType ());
						...
						if (ucos.Count > 0) {
							...
							lookupUnmanagedFunctionMethod.AddParameter (abr.System_String);
