...
namespace MonoTouchFixtures.NetworkExtension {
	...
	public class OnDemandTest {
		...
		{
			ClassicAssert.Null (rule.DnsSearchDomainMatch, "DnsSearchDomainMatch");
			...
			ClassicAssert.Null (rule.DnsSearchDomainMatch, "DnsSearchDomainMatch-3");

			ClassicAssert.Null (rule.DnsServerAddressMatch, "");
			...
			ClassicAssert.Null (rule.DnsServerAddressMatch, "DnsServerAddressMatch-3");
			...
			ClassicAssert.Null (rule.ProbeUrl, "ProbeUrl");
			using (var url = new NSUrl ("http://www.xamarin.com")) {
				rule.ProbeUrl = url;
				ClassicAssert.AreSame (url, rule.ProbeUrl, "ProbeUrl-2");
			}
			rule.ProbeUrl = null;
			ClassicAssert.Null (rule.ProbeUrl, "ProbeUrl-3");

			ClassicAssert.Null (rule.SsidMatch, "SsidMatch");
			rule.SsidMatch = new string [] { "1", "2", "3" };
			Assert.That (rule.SsidMatch.Length, Is.EqualTo (3), "SsidMatch-2");
			rule.SsidMatch = null;
			ClassicAssert.Null (rule.SsidMatch, "SsidMatch-3");
		}
		...
		public void NEOnDemandRuleEvaluateConnection ()
		{
			...
			using (var rule = new NEOnDemandRuleEvaluateConnection ()) {
				...
				OnDemandRule (rule);
				// 
				ClassicAssert.IsNull (rule.ConnectionRules, "ConnectionRules");
				rule.ConnectionRules = new NEEvaluateConnectionRule [] {
					new NEEvaluateConnectionRule ()
				};
				ClassicAssert.IsNotNull (rule.ConnectionRules, "ConnectionRules-2");
				rule.ConnectionRules = null;
				ClassicAssert.IsNull (rule.ConnectionRules, "ConnectionRules-3");
			}
		}
		...
		public void EvaluateConnectionRule_Default ()
		{
			...
			using (var rule = new NEEvaluateConnectionRule ()) {
				...
				ClassicAssert.Null (rule.MatchDomains, "MatchDomains");
				ClassicAssert.Null (rule.ProbeUrl, "ProbeUrl");
				ClassicAssert.Null (rule.UseDnsServers, "UseDnsServers");
			}
		}
		...
		public void EvaluateConnectionRule ()
		{
			...
			using (var rule = new NEEvaluateConnectionRule (new [] { "xamarin.com" }, NEEvaluateConnectionRuleAction.ConnectIfNeeded)) {
				...
				ClassicAssert.Null (rule.UseDnsServers, "UseDnsServers-3");
			}
		}
	}
}
