...
#nullable enable
namespace CoreImage {
	/// <summary>A <see cref="T:CoreImage.CIFilter" /> whose pixels are randomly colored.</summary><remarks><para>The <see cref="T:CoreImage.CIRandomGenerator" /> is idempotent: it always returns the same data. To retrieve a different random pattern, developers
	/// must retrieve the data from a random offset. For instance:</para><example><code lang="csharp lang-csharp"><![CDATA[
	...
	/// ]]></code></example><para>Each of the color components is independently uniformly distributed.</para><para>The following example shows this filter in use:</para><example><code lang="csharp lang-csharp"><![CDATA[
	...
	/// </para><para><img href="~/xml/CoreImage/_images/random_generator.png" alt="Result of applying the filter." /></para></remarks>
	public partial class CIRandomGenerator : CIFilter, ICIRandomGeneratorProtocol, ICIFilterProtocol {
		/// <summary>Creates a new <see cref="CIRandomGenerator" /> with default values.</summary>
		...
		/// </remarks>
		[BindingImpl (BindingImplOptions.GeneratedCode | BindingImplOptions.Optimizable)]
		protected internal CIRandomGenerator (NativeHandle handle) : base (handle)
		{
		}
		...
		protected CIRandomGenerator (NSObjectFlag t) : base (t)
		{
		}
		...
		public CIRandomGenerator (NSCoder coder) : base (NSObjectFlag.Empty)
		{
			if (coder is null)
		...
		// CIRandomGeneratorProtocol protocol members 
