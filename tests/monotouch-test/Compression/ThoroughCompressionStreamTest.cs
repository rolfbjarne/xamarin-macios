...
namespace MonoTouchFixtures.Compression {
	...
	public class ThoroughCompressionStreamTest {
		...
		void DecodeRealFile (CompressionAlgorithm algorithm, string compressedFile, string uncompressedFile)
		{
			...
			output.Seek (0, SeekOrigin.Begin);
			ClassicAssert.AreNotEqual (0, output.Length, "Stream length should not be 0,");
			ClassicAssert.IsTrue (compare_buffers (File.ReadAllBytes (uncompressedFile), output.GetBuffer (), (int) output.Length), "Streams are not equal.");
			decompressing.Close ();
			output.Close ();
		}
