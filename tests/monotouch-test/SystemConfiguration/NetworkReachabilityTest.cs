	...
	public class NetworkReachabilityTest {
		...
		public void CtorNameAddress ()
		{
			using (var nr = new NetworkReachability ("apple.com")) {
				...
				ClassicAssert.IsTrue (nr.TryGetFlags (out flags));
		...
		public void CtorIPAddress ()
		{
			using (var nr = new NetworkReachability (IPAddress.Loopback)) {
				...
				ClassicAssert.IsTrue (nr.TryGetFlags (out flags), "#1");
				...
				ClassicAssert.True ((flags & NetworkReachabilityFlags.Reachable) != 0, "Reachable");
			}

			using (var nr = new NetworkReachability (new IPAddress (new byte [] { 10, 99, 99, 99 }))) {
				NetworkReachabilityFlags flags;

				ClassicAssert.IsTrue (nr.TryGetFlags (out flags), "#2");
			...
			using (var nr = new NetworkReachability (IPAddress.IPv6Loopback)) {
				...
				ClassicAssert.IsTrue (nr.TryGetFlags (out flags), "#3");
			...
			using (var nr = new NetworkReachability (IPAddress.Parse ("2001:4860:4860::8844"))) {
				...
				ClassicAssert.IsTrue (nr.TryGetFlags (out flags), "#4");
		...
		void CheckLoopbackFlags (NetworkReachabilityFlags flags, string number, bool has_address)
		{
			...
			ClassicAssert.AreEqual (noFlags, otherFlags, $"#{number} No other flags: {flags.ToString ()}");
		}
		...
		public void Schedule ()
		{
			...
			ClassicAssert.IsTrue (defaultRouteReachability.Schedule (CFRunLoop.Main, CFRunLoop.ModeDefault), "Schedule");
			ClassicAssert.IsTrue (defaultRouteReachability.Unschedule (CFRunLoop.Main, CFRunLoop.ModeDefault), "Unschedule");
		}
		...
		public void SetNotification ()
		{
			...
			ClassicAssert.AreEqual (StatusCode.OK, statusCode, "SetNotification should succeed");

			// Test clearing the notification (this should free the GCHandle)
			statusCode = reachability.SetNotification (null);
			ClassicAssert.AreEqual (StatusCode.OK, statusCode, "SetNotification(null) should succeed");

			// Test setting notification again after clearing
			statusCode = reachability.SetNotification ((flags) => {
			});
			ClassicAssert.AreEqual (StatusCode.OK, statusCode, "SetNotification should succeed again");
		...
		public void SetNotification_GCHandleFreed ()
		{
			...
			ClassicAssert.IsTrue (collectedCount > 0, $"Expected at least one NetworkReachability instance to be collected, but {collectedCount} were collected");
		}
