	...
	public class LocalizationStringTest : TestBase {
		...
		public void AllSupportedTranslations (string culture, string errorMessage)
		{
			...
			try {
				...
				ClassicAssert.IsTrue (isTranslated, $"Should contain \"{errorMessage}\", but instead has value: \"{Engine.Logger.ErrorEvents [0].Message}\"");
			} finally {
		...
		public void SpecificErrorTranslation (string culture)
		{
			...
			try {
				ClassicAssert.IsFalse (string.IsNullOrEmpty (errorCode), "Error code is null or empty");
		...
		public void UpdatedResources ()
		{
			...
			ClassicAssert.IsEmpty (errorsNotInResources, $"The following error(s) were found in MSBStrings.resx but not through the MSBStrings resource. Try to recompile the msbuild project and then the test project\n{errorsNotInResources}");
			ClassicAssert.IsEmpty (errorsNotInResx, $"The following error(s) were found in the MSBStrings resource but not in MSBStrings.resx. Try to recompile the msbuild project and then the test project\n{errorsNotInResx}");
		}
	}
}
