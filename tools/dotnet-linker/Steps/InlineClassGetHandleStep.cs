...
public class InlineClassGetHandleStep : AssemblyModifierStep {
	...
	protected override void TryProcess ()
	{
		...
		if (strictMode && Configuration.Application.Registrar == Bundler.RegistrarMode.Dynamic) {
			Report (ErrorHelper.CreateError (Configuration.Application, 2260, null, Errors.MX2260));
		}
	...
	protected override bool ProcessMethod (MethodDefinition method)
	{
		...
		foreach (var instr in method.Body.Instructions) {
			...
			if (ldstr.OpCode != OpCodes.Ldstr) {
				Report (ErrorHelper.CreateWarning (Configuration.Application, 2261, method, Errors.MX2261, FormatMethod (method), ldstr));
				continue;
			}
			if (ldstr.Operand is not string objectiveCClassName) {
				Report (ErrorHelper.CreateWarning (Configuration.Application, 2261, method, Errors.MX2261, FormatMethod (method), ldstr.Operand));
				continue;
			}
