	...
	public partial class Application
	{
		...
		public void AddAssemblyBuildTarget (string value)
		{
			...
			if (assembly_build_targets.ContainsKey (assembly_name))
				throw ErrorHelper.CreateError (101, "The assembly '*' is specified multiple times in --assembly-build-target arguments.", assembly_name);
			...
			foreach (var target in Targets) {
				...
				foreach (var assembly in target.Assemblies) {
					...
					if (build_target == null) {
						...
						exceptions.Add (ErrorHelper.CreateError (98, "No assembly build target was specified for '{0}'.", assembly.Identity));
						continue;
					}
			...
			if (!IsDualBuild && !IsExtension) {
				...
				if (HasFrameworks) {
					foreach (var abi in AllArchitectures) {
						...
						abis.Add (abi);
						ErrorHelper.Warning (104, "There is at least one extension that builds for '*'. The main app must also build for this architecture if assemblies are compiled into frameworks, so it has automatically been enabled.", abi.AsString ());
					}
				}
			}
