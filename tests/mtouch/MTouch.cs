...
";
				...
				mtouch.CreateTemporaryApp (extraCode: codeExe, extraArg: $"-r:{Quote (dllPath)}");
				mtouch.Linker = MTouchLinker.LinkSdk;
				mtouch.Debug = false;
				mtouch.CustomArguments = new string [] { "--dlsym:+A.dll", "--dlsym:-testApp.exe" };
				mtouch.AssertExecute (MTouchAction.BuildDev, "build");

				var symbols = ExecutionHelper.Execute ("nm", $"-gUj {Quote (mtouch.NativeExecutablePath)}", hide_output: true).Split ('\n');
