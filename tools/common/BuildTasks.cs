			...
			if (started_task.TrySetResult (true)) {
				...
				try {
					...
					if (IsUptodate) {
						...
						try {
							Log ("Executing task #{1}: {0}", GetType ().Name, ID);
							watch.Start ();
							await ExecuteAsync ();
							watch.Stop ();
							Log ("Completed task #{1}: {0} in {2} s", GetType ().Name, ID, watch.Elapsed.TotalSeconds);
					...
					Log ("Completed task #{1}: {0} in {2} s with exception: {3}", GetType ().Name, ID, watch.Elapsed.TotalSeconds, e.Message);
					completed_task.SetException (e);
					throw;
				}
			} else {
				Log ("Waiting for started task #{1}: {0}", GetType ().Name, ID);
				await completed_task.Task;
				Log ("Waited for started task #{1}: {0}", GetType ().Name, ID);
			}
		}
